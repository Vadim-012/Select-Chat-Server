...
bool validateBuffer(char *buffer, ssize_t bytesRead, size_t maxSize);
...
int32_t main(int argc, char *argv[]){
    ...
    checkSyscall(sigaction(SIGPIPE, &sa, NULL), "Syscall: sigaction SIGPIPE");
    ...
    addr.sin_port = (uint16_t)((uint32_t)(finalPort >> 8U) | (uint32_t)(finalPort << 8U));
    ...
    for(size_t i = 0; i < (size_t)maxClient; i++){
    ...
    if(FD_ISSET(serverFd, &ReadFds))
    {
        if((size_t)currentClientsCount < (size_t)maxClient)
        {
            ...
            if(client >= 0)
            {
                int32_t flags = fcntl(client, F_GETFL, 0);
                if(flags >= 0)
                {
                    fcntl(client, F_SETFL, flags | O_NONBLOCK);
                }

                for(size_t i = 0U; i < (size_t)maxClient; i++){
                    if(clientArray[i].fd == -1)
                    {
        ...
        else
        {
            ...
            if(rejectedClientFd >= 0)
            {
                ...
                (void)write(rejectedClientFd, rejectionMessage, sizeof(rejectionMessage) - 1U);
...
void disconnectClient(size_t clientIndex, Client *const clientArray, size_t maxClient, int32_t historyFd){
    ...
    {
        FD_CLR(clientArray[clientIndex].fd, &MasterFds);
        close(clientArray[clientIndex].fd);
        clientArray[clientIndex].fd = -1;
    }

    clientArray[clientIndex].name[0] = '\0';
